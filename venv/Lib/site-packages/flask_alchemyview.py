...
def _exception_to_dict(error):
    ...
    if isinstance(error, IntegrityError):
        m = re.search(r'(Key) \((\w+)\)=\(([^)]*)\) already exists',
                      str(error.orig))
        if m:
            return {'message': _("'%(key)' already exists", key=m.group(2)),
                    'errors': {m.group(2): _('Already exists')}}
    elif isinstance(error, colander.Invalid):
        return {'errors': error.asdict(),
    ...
    return {'message': _('Unknown error'), 'errors': {}}
...
class BadRequest(HTTPException):
    ...
    def __init__(self, code, data):
        ...
        self.code = code
        super(BadRequest, self).__init__(self.data['message'])
...
class AlchemyView(FlaskView):
    ...
    def _get_item(self, id):
        ...
        primary_key_name = primary_key[0][0]
        if primary_key_type not in (int, str, str):
    ...
    def index(self):
        ...
        try:
            ...
            return self._response({'message': _('Invalid limit')},
                                  'index',
                                  400)
        if limit > 100:
            limit = 10
        try:
            ...
            return self._response({'message': _('Invalid offset')},
                                  'index',
                                  400)
        try:
            ...
            return self._response({'message': _('Invalid sortby')},
                                  'index',
                                  400)
        try:
            direction = str(request.args.get('direction', self.sort_direction))
        except:
            return self._response({'message': _('Invalid direction')},
                                  'index',
                                  400)
